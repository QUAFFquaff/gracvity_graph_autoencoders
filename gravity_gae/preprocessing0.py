...
def mask_test_edges_general_link_prediction(adj, graph, feature_arr, test_percent=10.,
    ...
    # adj.eliminate_zeros()
    graph_ind_to_id = list(graph.keys())
    if np.array_equal(adj, adj.T):
        print("symmetric")
    else:
        print('not symmetric')
    edges_positive, values, _ = sparse_to_tuple(adj)
    np.savetxt('edges_positive.txt', edges_positive,fmt="%d")
    ...
    test_edge_idx = edges_positive_idx
    # positive test edges
    ...
    # 我们的版本
    train_edges = np.delete(edges_positive, [val_edge_idx], axis = 0)

    np.savetxt('train_edges.txt', train_edges,fmt="%d")
    np.savetxt('values.txt', values,fmt="%d")
